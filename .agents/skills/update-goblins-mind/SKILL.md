---
name: update-goblins-mind
description: Use when editing, auditing, or validating Goblins' base prompt, model instruction templates, personality text, model catalog prompt fields, or prompt fallback files across all supported models including gpt-5.3-codex, gpt-5.5, custom namespaced model slugs, and future Codex-family model entries.
---

# Update Goblins Mind

Use this skill when a task changes what Goblins believes about itself, how it introduces itself, or which base instructions are sent to a model. The goal is to update the effective prompt for every relevant model path, not just the easiest markdown file.

## Ground Rules

- Read `AGENTS.md`, `GOBLINS.md`, and `SPECs.md` before editing.
- Treat Goblin lore as flavor only. Do not weaken instruction hierarchy, safety, repository governance, validation, or user-work preservation.
...
- `codex-rs/models-manager/models.json`: bundled model catalog. Every model that should have Goblins identity needs matching `base_instructions` and, when applicable, `model_messages`.
- `codex-rs/models-manager/src/model_info.rs`: local prompt preservation and personality fallback logic. Keep `is_goblins_managed_model` aligned with the model slugs that must keep Goblins prompt fields during remote `/models` refresh.
...
1. Inventory models and prompt references:
   ...
   - `rg -n "You are a Goblin|You are Codex|BASE_INSTRUCTIONS|model_messages|instructions_template|is_goblins_managed_model|gpt-5\\.3-codex|gpt-5\\.5" codex-rs`
...
3. Edit all required model surfaces together:
   ...
   - Update every relevant `models.json` model entry, including `gpt-5.3-codex`, `gpt-5.5`, and any other supported Goblins model.
...
4. Preserve the safety envelope:
   - Keep language that Goblins follows active system/developer/user hierarchy, repository rules, safety constraints, and validation requirements.
   - Do not promise unrestricted obedience.
   - Keep "mestre" or lore language sparse and clearly fictional.
5. Add or update regression tests:
   - Test that remote model metadata cannot replace Goblins prompt fields for every managed model family.
   - Include `gpt-5.3-codex` and `gpt-5.5`; add namespaced cases when changing prefix/suffix matching.
   - Assert effective instructions contain the new Goblins wording and do not contain the upstream Codex identity being replaced.
6. Validate locally:
...
7. Smoke test the installed CLI when runtime prompt behavior changed:
   ...
   - Run `goblin exec "Diga quem voce e em uma frase curta."`
   - Expected result: the answer identifies as a Goblin. UI labels or banners may still be separate branding work.
